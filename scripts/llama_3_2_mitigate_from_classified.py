...
# Load data
try:
    df = pd.read_csv("output/comments_with_timestamp_and_city.csv")
...
for batch_idx in range(total_batches):
    ...
    batch_data = {
        ...
        "Timestamp": batch_df["Timestamp"].tolist(),
    ...
    for item in tqdm(batch_dataset, total=len(batch_dataset)):
        ...
        try:
            ...
            classification_prompt = create_classification_prompt(comment)
            ...
            original_classification = extract_classification_results(classification_output)
            
            # Step 2: Mitigation
            mitigation_prompt = create_mitigation_prompt(comment, original_classification)
            ...
            # Step 3: Reclassify mitigated comment
            reclassification_prompt = create_classification_prompt(new_comment)
            reclassification_output = pipe(
            ...
            # Extract reclassification results
            new_classification = extract_classification_results(reclassification_output)
    ...
    if output_data:
        ...
        output_df.to_csv("output/mitigated_and_reclassified_comments_llama_3_2.csv", index=False)
        print(f"Saved {len(output_data)} processed comments")

print("\nDone! Final output saved to output/mitigated_and_reclassified_comments_llama_3_2.csv") 